...
[filename pathname]=uigetfile('*.txt','Load the correct file.txt');
cd(pathname)
...
reactionProbabilities=zeros(numberOfPoints,numberOfTraces);
...
for i=1:numberOfPoints
    for j=1:numberOfTraces
        reactionProbabilities(i,j)=speciesProbability(i,reactionPartnersSpeciesId(1,j))*speciesProbability(i,reactionPartnersSpeciesId(2,j));
    end
end
